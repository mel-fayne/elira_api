...
class TechnicalProfileView(APIView):
    ...
    def post(self, request):
        dev_details = gitScrapper(request.data['git_username'])
    ...
    def patch(self, request, *args, **kwargs):
        profile = TechnicalProfile.objects.filter(id=self.kwargs['student_id']).first()
        dev_details = gitScrapper(request.data['git_username'])
        serializer = TechnicalProfile(profile, data=dev_details, partial=True)
        serializer.is_valid(raise_exception=True)
        serializer.save()
        return Response(serializer.data)
    
def gitScrapper(git_username):
    dev_details = {}
    # get commits and PRs
